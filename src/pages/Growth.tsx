...
const trendData: Record<Period, {
  labels: string[]; total: number[]; patriarchs: number[]; muses: number[];
  constellations: number[]; monthlyAvg: number; yearlyAvg: number;
}> = {
  week: {
    ...
    monthlyAvg: 9480, yearlyAvg: 7840,
  },
  month: {
    ...
    monthlyAvg: 70080, yearlyAvg: 54280,
  },
  year: {
    ...
    monthlyAvg: 67420, yearlyAvg: 67420,
  },
};
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ minWidth: 420, width: '100%', height: 'auto', display: 'block' }}>
        ...
        {showRefs && (
          <>
            <line x1={pad.l} x2={W - pad.r} y1={toY(data.monthlyAvg)} y2={toY(data.monthlyAvg)} stroke={GOLD}   strokeWidth="1" strokeDasharray="5,3" />
            <line x1={pad.l} x2={W - pad.r} y1={toY(data.yearlyAvg)}  y2={toY(data.yearlyAvg)}  stroke={PURPLE} strokeWidth="1" strokeDasharray="5,3" />
          </>
        )}
        <path d={areaPath} fill={`url(#${gid})`} />
        <path d={linePath} fill="none" stroke={ACCENT} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        {pts.map((p, i) => <circle key={i} cx={p.x} cy={p.y} r="3" fill={ACCENT} opacity="0.85" />)}
